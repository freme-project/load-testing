...
This test still uses JMeter - we decided to stop jmeter support as it does not allow us to exactly what we want to test.

If you still want to use this test, read the JMETER-README.md on a guide on how to start and interpret this test.

"""
if os.getcwd().endswith("load-testing"):
    os.chdir(os.getcwd()+"/jmeter-test")
...
k=0
for i in range(min_threads, max_threads, step):
...
for n in range(min_threads, max_threads, step):
    current=table[:n]
